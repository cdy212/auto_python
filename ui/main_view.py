...
# (모든 내용은 이전과 동일하며, 위에 공유된 main.py, core/recorder.py, core/controls.py 와
# ui/actions_frame.py 수정에 따라 내부적으로 개선된 기능을 활용합니다.)

# JobRunner의 triggered_events 생성 부분은 변경 없음:
# event_data = {
#     "condition": condition,
#     "target_window": { # 이 'target_window'가 controls.py로 전달됩니다.
#         "title": window['title'],
#         "rect": window['search_rect'], # 'search_rect'는 이제 클라이언트 영역 절대 좌표입니다.
#     }
# }
# triggered_events.append(event_data)

# ActionExecutor에서 controls.execute_action_sequence 호출 부분도 변경 없음:
# controls.execute_action_sequence(
#     actions=event['condition'].actions,
#     target_window=event['target_window'] # 이 부분은 여전히 동일하게 전달
# )

# (이 외의 main_view.py의 다른 내용은 이전 버전과 동일하게 유지됩니다.)
...
class VisualOverlay:
    """화면에 프로세스 위치를 사각형으로 표시하고, 이미지 탐지 결과를 붉은색으로 표시하는 투명 오버레이 창"""
    def __init__(self, root, stop_callback):
        ...
        self.image_highlight_timer = None
            ...
            while not overlay_queue.empty():
                ...
                if item['type'] == 'process_windows':
                    ...
                    for window_info in item['data']:
                        ...
                        text_id = self.canvas.create_text(x1 + 5, y1 - 25, text=title, font=("Arial", 12, "bold"), 
                                                fill="#00FF00", anchor="nw")
                        self.process_rect_ids.append(text_id)
                
                elif item['type'] == 'found_image':
                    rect, similarity = item['data']
                    x1, y1, x2, y2 = rect
                    
                    self.canvas.delete("image_highlight")
                    self.canvas.create_rectangle(x1, y1, x2, y2, outline="#FF0000", width=4, tags="image_highlight")
                    sim_text = f"Sim: {similarity*100:.1f}%"
                    self.canvas.create_text(x1, y1 - 15, text=sim_text, font=("Arial", 10, "bold"), fill="red", anchor="sw", tags="image_highlight")

                    if self.image_highlight_timer:
                        self.root.after_cancel(self.image_highlight_timer)
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if target_pids:
                ...
                for hwnd in hwnds[:2]: # 최대 2개의 창만 추적
                    full_rect = win32gui.GetWindowRect(hwnd) # 전체 창의 절대 좌표 (오버레이 표시용)
                    
                    # 검색 영역은 클라이언트 영역의 절대 좌표를 사용
                    ...
                    if condition.type == "Image Similarity":
                        ...
                        if image_path and os.path.exists(image_path) and tracked_windows:
                            for window in tracked_windows:
                                # 'search_rect'는 이제 클라이언트 영역 절대 좌표
                                ...
                                if found:
                                    ...
                                    event_data = {
                                        "condition": condition,
                                        "target_window": { # 이 정보를 controls.py로 전달합니다.
                                            "title": window['title'],
                                            "rect": window['search_rect'], # 클라이언트 영역 절대 좌표
                                        }
                                    }
                                    triggered_events.append(event_data)
            
            if triggered_events:
                self.action_queue.put(triggered_events)
        ...
        while not self.stop_event.is_set():
            try:
                ...
                for event in events_to_process:
                    ...
                    controls.execute_action_sequence(
                        actions=event['condition'].actions,
                        target_window=event['target_window'] # 'target_window'는 클라이언트 영역 rect를 포함합니다.
                    )
            except queue.Empty:
                continue
...
class AutomationUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Automation Project UI (v8.2 - Executor Added)")
    ...
    def create_widgets(self):
        ...
        main_frame.grid_columnconfigure(1, weight=3); main_frame.grid_columnconfigure(0, weight=2)
    ...
    def _create_left_panel(self, parent):
        ...
        saved_jobs_frame = ttk.LabelFrame(left_frame, text="Saved Jobs")
        saved_jobs_frame.grid(row=0, column=0, sticky="ew", pady=(0, 10))
    ...
    def _create_right_panel(self, parent):
        ...
        right_frame.rowconfigure(2, weight=1); right_frame.columnconfigure(0, weight=1)
        ...
        self.conditions_frame.grid(row=1, column=0, sticky="nsew", pady=10)
        ...
        self.actions_frame.grid(row=2, column=0, sticky="nsew", pady=10)
        logs_frame = ttk.LabelFrame(right_frame, text="Real-time Logs")
        logs_frame.grid(row=3, column=0, sticky="sew", pady=(10, 0)); logs_frame.rowconfigure(0, weight=1); logs_frame.grid_columnconfigure(0, weight=1)
    ...
    def start_tracking(self):
        ...
        if not self.current_job:
            self.log("ERROR: No job selected.")
            return
    ...
    def stop_tracking(self):
        if self.job_runner and self.job_runner.is_alive():
            self.log(f"Stopping job runner...")
            self.job_runner.stop(); self.job_runner = None
        
        if self.action_executor and self.action_executor.is_alive():
            self.log("Stopping action executor...")
            self.action_executor.stop(); self.action_executor = None
    ...
    def on_process_select(self, event=None):
        ...
        if is_saved:
            for name, job_obj in self.saved_jobs.items():
                if job_obj.process_name == full_process_name:
                    found_job_name = name
                    break
        if found_job_name:
            self.load_job_into_ui(found_job_name)
        else:
            self.current_job = None
            self.job_name_entry.delete(0, tk.END)
            self.job_name_entry.insert(0, base_name)
            self.process_name_entry.delete(0, tk.END)
            self.process_name_entry.insert(0, full_process_name)
    ...
    def load_job_into_ui(self, job_name):
        ...
        if self.current_job:
            self.job_name_entry.delete(0, tk.END)
            self.job_name_entry.insert(0, self.current_job.name)
            self.process_name_entry.delete(0, tk.END)
            self.process_name_entry.insert(0, self.current_job.process_name)
    ...
    def save_current_job(self):
        ...
        if not job_name or not process_name:
            self.log("ERROR: Job Name and Process Name are required.")
            return
        if self.current_job and self.current_job.name == job_name:
            self.current_job.process_name = process_name
        else:
            new_job = Job(name=job_name, process_name=process_name)
            if self.current_job:
                new_job.conditions = self.current_job.conditions
    ...
    def delete_job(self):
        if not self.saved_jobs_listbox.curselection():
            self.log("⚠️ No saved job selected to delete.")
            return
    ...
    def on_closing(self):
        if self.job_runner or self.action_executor:
            self.stop_tracking()
        self.root.destroy()