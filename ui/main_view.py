...
action_queue = queue.Queue() # <<< 추가: Action 실행을 위한 큐
        ...
        while not self.stop_event.is_set():
            ...
            if target_pids:
                ...
                for hwnd in hwnds[:2]:
                    ...
                    search_rect = (left, top, right, bottom)
                        ...
                        if image_path and os.path.exists(image_path) and tracked_windows:
                            for window in tracked_windows:
                                ...
                                if found:
                                    ...
                                    # 실행할 Action 작업 목록에 추가
                                    event_data = {
                                        "condition": condition,
                                        "target_window": {
                                            "title": window['title'],
                                            "rect": window['search_rect'],
                                        }
                                    }
                                    triggered_events.append(event_data)
            
            # <<< 수정: 발견된 모든 작업을 Action 큐에 한꺼번에 넣음 >>>
            if triggered_events:
...
class AutomationUI:
    def __init__(self, root):
        self.root = root; self.root.title("Automation Project UI (v8.2 - Executor Added)")
        self.root.geometry("1280x800")
        self.all_processes = []; self.saved_jobs = config_manager.load_jobs()
        self.current_job = None; self.job_runner = None; self.overlay_window = None
        self.action_executor = None # Action 실행 스레드 객체
        ...
        self.check_feedback_queue(); self.refresh_process_list()
    ...
    def start_tracking(self):
        ...
        # <<< 수정: JobRunner와 ActionExecutor 모두 시작 >>>
    ...
    # (setup_styles, create_widgets, 및 나머지 UI 로직 함수는 이전 코드와 동일)
    # ... (생략) ...
    ...
    def _create_right_panel(self, parent):
        ...
        self.actions_frame = ActionsFrame(right_frame)
    ...
    def on_closing(self):
        if self.job_runner: self.stop_tracking()
        self.root.destroy()